...
**Active Project:** Neuracode AI: Python & AI Öğrenme Ekosistemi
**Project File:** `[[01_Projeler/Neuracode AI/Neuracode AI_Ana_Plan.md]]`
**Description:** A gamified Android-based Python learning platform for kids featuring "Tech-Trax Rover" (Smart Farming). It runs smoothly on low-end devices with zero-keyboard input (drag-and-drop logic) and uses TinyML for optimized AI capabilities.

Whenever I ask for coding help, project architecture, or design advice, prioritize the principles outlined in this master plan (low system requirements, Material Design 3, gamified learning loop, edge AI).
...
### Çalışma Tarzı ve Sürüm Takibi (ÖNEMLİ)
1. **YAML Frontmatter:** Tüm projelere ve notlara oluşturulurken/düzenlenirken daima YAML frontmatter (`title`, `version`, `date`, `tags`, `status`, `related`) eklenecektir.
2. **Sürüm Bağlantıları (Versioning):** Bir proje veya plan güncellendiğinde eski metin silinmeyecektir. Yeni bir vX dosyası açılacak ve dosyaların başına/sonuna `Önceki sürüm: [[v1]]` ve `Sonraki sürüm: [[v2]]` şeklinde kronolojik wikilink bağlantıları eklenecektir.
3. **Zettelkasten / Kavram Çıkarımı:** Metin içindeki tekrar eden önemli mimari kararlar veya teknik yapılar (Örn: `Tech-Trax Rover`, `TinyML`) orijinal dosyada sadece `[[wikilink]]` bırakılarak `03_Kaynaklar/` altına tekil notlar olarak taşınacaktır.
4. **Non-destructive:** Orijinal not içerikleri veya kullanıcının düşünceleri asla silinmeyecektir, sadece zenginleştirilecek ve bağlanacaktır.
